...
**B2d. (tùy chọn) Trang SAYA — `ADJUST_SAYA_QUERY` + `META_SAYA_AD_ACCOUNT_ID`**
Dashboard sinh thêm trang **saya.html** (URL `/saya` trên Pages) nếu `.env` có `ADJUST_SAYA_QUERY` —
query y hệt `ADJUST_REPORT_QUERY` nhưng app_token = app **M002 - Saya**, `utc_offset=-07:00` khớp
account Meta Saya. Thêm `META_SAYA_AD_ACCOUNT_ID` (act_… của Saya, cùng system-user token) để có
bảng Campaign/Creative với CTR, hook-rate như Cardia. Cost TikTok nằm sẵn trong Adjust qua partner
link → **không cần TikTok API** (creative-level TikTok chờ platform mới).
